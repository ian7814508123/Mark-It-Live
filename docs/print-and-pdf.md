...
本文件詳細說明  Mark It Live 的物理列印、強制分頁、多文件合併下載/列印，以及客戶端 PDF 合併工具的實作機制與維護說明。
...
### 📊 圖表 (Diagram) 列印與特色主題高保真保留 [NEW]
本系統特別針對 Mermaid 模式與預覽區內建圖表，設計了最優雅的高保真列印機制，對齊專案的「美學列印分流」大原則：

1. **美學列印分流 (Theme-Adaptive Printing)**：
   - **預設主題 (DEFAULT Theme) 列印**：Mermaid 圖表在列印時會採取**「極簡黑白」**風格（節點背景為淺灰色/純白，文字與連線為高對比灰黑，無任何彩色），避免黑白影印時色彩失焦。
   - **客製特色主題列印**：圖表在列印時會**完整保留當前所選特色主題降級為淺色模式時的專屬色彩與字型**（如 Cosmic 暖紫、Nordic Forest 的深林綠等），在輸出為 PDF 檔案時保留高強度的視覺張力，達到極致的氛圍感。
2. **使用者自訂色 100% 護航 (classDef High Fidelity)**：
   - 不論是 DEFAULT 還是特色主題，使用者在 Markdown 圖表代碼中利用 `classDef` 或 `style` 自訂的節點顏色與屬性，在列印時皆會被 **100% 忠實保留**（完全不受列印降級與洗淨機制干擾）。
3. **列印殘留零容忍 (No Style Residues)**：
   - 系統在 `@media print` 階段會強制拔除 `.diagram-block-container` 的 `box-shadow` 陰影、邊框背景等，確保 SVG 圖表乾淨無瑕地融入 A4 白紙背景，防止圖表外圍產生突兀的色塊補丁。
